...
describe Devdigest::Credentials do
  describe '.token' do
    let(:netrc) { stub :netrc, :[] => %w( arthur@dent.com towel ) }
    let(:label) { stub :label }
    subject { Devdigest::Credentials.token(netrc, label) }
    it { should eq 'towel' }
  end
  ...
  describe '#token' do
    let(:netrc) { stub :netrc, :[] => %w( arthur@dent.com towel ) }
    let(:label) { stub :label }
    subject { Devdigest::Credentials.new(netrc, label).token }
  ...
  describe '.save' do
    ...
    it 'saves the token' do
      ...
      Devdigest::Credentials.save 'login', 'new token', netrc, label
    end
  end
  ...
  describe '#save' do
    ...
    subject { Devdigest::Credentials.new(netrc, label) }
